...
public class GTMTE_FluidMultiStorage2 extends GT_MetaTileEntity_MultiBlockBase {
	...
	private final Block MULTI_HATCH = Block_TFFTMultiHatch2.getInstance();
	private final int CASING_TEXTURE_ID = 176;

	private MultiFluidHandler2 mfh;
	private HashSet<TE_TFFTMultiHatch2> singleHatches = new HashSet<>();

	private int runningCost = 0;
	private boolean doVoidExcess = false;

	public GTMTE_FluidMultiStorage2(int aID, String aName, String aNameRegional) {
		super(aID, aName, aNameRegional);
	}

	public GTMTE_FluidMultiStorage2(String aName) {
		super(aName);
	}
	
	@Override
	public IMetaTileEntity newMetaEntity(IGregTechTileEntity var1) {
		return new GTMTE_FluidMultiStorage2(super.mName);
	}
	...
	public boolean checkRecipe(ItemStack guiSlotItem) {
		...
		if(singleHatches.size() > 0 && super.mInputHatches.size() == 0 && super.mOutputHatches.size() == 0) {
			return true;
		}
	...
	public boolean checkMachine(IGregTechTileEntity thisController, ItemStack guiSlotItem) {
				...
				if(X == -1 && X == 1 && Y == -1 && Y == 1) {
					if (!super.addMaintenanceToMachineList(currentTE, CASING_TEXTURE_ID)
						...
						if (thisController.getBlockOffset(offset.x(), offset.y(), offset.z()) == CASING) {
							...
							final TE_TFFTMultiHatch2 mh =
									(TE_TFFTMultiHatch2) thisController.getWorld().getTileEntity(
											thisController.getXCoord() + offset.x(), 
											thisController.getYCoord() + offset.y(),
											thisController.getZCoord() + offset.z());
							singleHatches.add(mh);
						}
						else {
		...
		if(formationChecklist) {
			...
			if(mfh == null) {
				mfh = new MultiFluidHandler2(capacityPerFluid);
			} else {
				if(mfh.getCapacity() != capacityPerFluid) {
					mfh = new MultiFluidHandler2(capacityPerFluid, mfh.getFluids());
				}
			}
			for(TE_TFFTMultiHatch2 mh : singleHatches) {
				mh.setMultiFluidHandler2(mfh);
			}
		}
